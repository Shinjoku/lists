...
int count = 0;			/* variavel que conta quantos produtos existem dentro do buffer*/
...
//verifica se buffer já esta cheio
	if ((count < SIZEOFBUFFER) || (rp!=(wp+1))) {
		...
		*wp = toAdd;
		//colocou no buffer incrementa o count;
		return 1;
	} else return 0;
}
...
int myremove() {
	...
	if ((count > 0) || (wp != rp)) {
		...
		//cosumio do buffer decrementa o count;
		return retValue;
	} else return 0;
}
...
void *produce(void *threadid)
{
	...
	printf("Produtor #%d não conseguiu %d vezes\n", threadid, cont_foi);
	fflush(stdout);
	pthread_exit(NULL);
}
...
void *consume(void *threadid)
{
	...
	printf("Consumidor #%d não conseguiu %d vezes\n", threadid, cont_foi);
...
int main(int argc, char *argv[]){
 ...
 // Não consegui utilizar o comando, pois algumas threads ficam para sempre o que trava o comando
