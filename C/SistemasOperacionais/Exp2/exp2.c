...
void Receiver(int queue_id)
{
	...
	for( count = 0; count < NO_OF_ITERATIONS; count++ ) {
		 ...
		 * Calcula a diferenca
	   	 */
            	...
            	//MODIFICAÇÃO :::::> delta -= receive_time.tv_sec  - data_ptr->send_time.tv_sec;
	 ...
	 * Exibe os resultados
	 */
	printf( "O tempo medio de transferencia: %f\n", total / NO_OF_ITERATIONS );
	fprintf(stdout, "O tempo maximo de transferencia: %f\n", max );
