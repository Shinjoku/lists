...
c.drawCentredString((this_page.x * 1.8)/3, this_page.y - 60, "SOLICITAÇÃO DE SERVIÇO")
...
for cont in sorted(content.items()): # cont = (key, value)
    ...
    if cont[1] == 'txtf':
        ...
        drawData(c, cont[0][1::], 'HEHE', pair(X, page_size.y - Y), cont[1])
        
        X += cont[1] + c.stringWidth(cont[0]) - 1 + blank
