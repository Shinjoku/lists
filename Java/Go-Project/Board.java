...
public class Board{
	...
	private boolean isPieceAt (int x, int y)
	{
		...
		{
			if (((Piece)pieces.getCursor ()).getX () == x && ((Piece) pieces.getCursor ()).getY () == y)
			{	
					result = true;
					break;
			}
	...
	public boolean removePiece (int x, int y)
	{
		...
		if (isPieceAt (x, y))
		{
			System.out.println ("REMOVE:" + ((Piece)pieces.getCursor()).getX () + " " + ((Piece)pieces.getCursor()).getY ());
			pieces.remove ();
			result = true;
		}
	...
	public boolean DeleteLibsOf (Piece p, Piece caller)
	{
		...
		while (loop == true)	
		{
			if (((Piece) pieces.getCursor ()).getChecked () == 1 && ((Piece) pieces.getCursor ()).getColor () == p.getColor ())
			{
				pieces.remove ();
		...
		{	
			while (result == true)	//checkNext returns the next node without moving cursor
			{
				if (((Piece)(pieces.getCursor ())).getColor () == 0)
					((Piece)(pieces.getCursor ())).paint (g, a, ((Piece)(pieces.getCursor ())).getX (), ((Piece)(pieces.getCursor ())).getY (), pblack);
				else if (((Piece)(pieces.getCursor ())).getColor () == 1)
					((Piece)(pieces.getCursor ())).paint (g, a, ((Piece)(pieces.getCursor ())).getX (), ((Piece)(pieces.getCursor ())).getY (), pwhite);
	...
	public void Score (Player p1, Player p2)
	{
		...
		//black_score = Score_Sub (min, min, 0);
